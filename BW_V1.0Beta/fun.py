...
def load_settings():
    ...
    for i in f:
        data = i.split('=')
        if data[0] == 'screen':
            screen_size = map(int, data[1].strip().split(';'))
        if data[0] == 'volume':
            volume = float(data[1].strip())
        if data[0] == 'fullscreen' and data[1].strip() == 'True':
            fullscreen = pygame.FULLSCREEN
    return screen_size, volume, fullscreen


def save_settings(screen, volume, fullscreen):
    f = open('data/settings/saved_settings.txt', 'w')
    f.write("screen={};{}\nvolume={}\nfullscreen={}".format(screen[0], screen[1], volume, fullscreen))
    f.close()
...
def win(c, sc, w, h):
    ...
    while running:
        ...
        pygame.draw.rect(sc, c.color, (0, 0, w, h))
